class Stories::ShowSupport
  ...
  def detail_timeline_chart
    [
      {
        text: 'Requirement',
        logs: working_logs.select(&:requirement?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
      {
        text: 'Design',
        logs: working_logs.select(&:design?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
      {
        text: 'Coding',
        logs: working_logs.select(&:coding?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
      {
        text: 'Testing',
        logs: working_logs.select(&:testing?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
      {
        text: 'Bug Fixing',
        logs: working_logs.select(&:bug_fixing?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
      {
        text: 'Release',
        logs: working_logs.select(&:release?).map{|e| {name: e.user_name, date: e.spent_on}}
      },
