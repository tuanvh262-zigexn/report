class Timecrowds::IndexSupport
  ...
  def time_entry_chart
    TimecrowdTimeEntry.joins(task: :category)
      .select("SUM(duration) AS durations, timecrowd_categories.title")
      .where(spent_on: params_search[:search_date].beginning_of_month..params_search[:search_date].end_of_month)
      .group("timecrowd_tasks.timecrowd_category_id")
      .map do |e|
        {
          title: e[:title],
          durations: convert_durations(e[:durations])
        }
      end
  end
  ...
  def params_search
    @params_search ||= begin
      {
        search_date: params_q.dig(:search_date)&.to_date || Date.current
      }
    end
  end
  ...
  def convert_durations durations
    hours = durations / 3600.0  # chia cho 3600 để ra giờ
  end
